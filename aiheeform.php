...
if(isset($_POST['submit']))
{
   ...
   {
       $vehicle=$_POST['checkbox'];
       $vehicles='';
       foreach($vehicle as $vehiclevalue)
       {
   ...
   else
   {
    ...
    $INSERT=mysql_query("INSERT INTO `student`(`stu_id`, `div_id`, `st_id`, `t_id`, `home_id`,
    `stu_name`, `stu_fname`, `image`, `addr`, `state`, `dist`, `city`, `pin`, `email`, `num`,
    `dob`, `categ`, `edu_pass`, `perc`, `year_pass`, `income`, `citizen`, `tenth`, `twelth`, 
    `graduate`, `board`, `institute`, `declar`, `checkbox`) VALUES ('$stu_name','$stu_fname','$fn','$addr') ");
...
<table Width="100%">
	...
	<tr><td  width="30%" style="font-size:30px;">Country:
	<select name="country" id="country" style="width:30%; padding:12px;>			
			<option value="0">Select country</option>
			<?php
			 $cnt=mysql_query("select * from countries order by name asc ");
			 $num=mysql_num_rows($cnt);
			 if($num>0)
			 {
			     $sr=0;
			 
			 while($arcnt=mysql_fetch_array($cnt))
			...
			
			 {
			     $sr++;
			      //$arcnt['id']);
			?>
						<option value="<?php echo $arcnt['id']; ?>@@<?php echo $arcnt['name']; ?>"<?php if($country==$arcnt['id']) echo 'checked'; ?>><?php echo $arcnt['name']; ?></option>
						<?php
			 }	
			 }
			?>
	...
	<tr>
		<td width="30%" style="font-size:30px;">
			...
			<select name="state" id="state" style="width:30%; padding:12px; >			
			<option value="0">Select state</option>
			<?php
			 $snt=mysql_query("select * from states order by name asc ");
			...
			
			 {
			     $sr++;
					...
					?>
						<option value="<?php echo $arcnt1['id']; ?>@@<?php echo $arcnt1['name']; ?>"<?php if($stateId==$arcnt1['id']) echo 'checked'; ?>><?php echo $arcnt1['name']; ?></option>
						<?php
			 }			
			 }
			?>
		...
		5.Distt :<select name="city" id="city" style="width:30%; padding:12px;>			
			<option value="">Select city</option>
			<?php
			 $sntcity=mysql_query("select * from cities order by name asc ");
			...
			
			 {
			     $sr++;
					...
					?>
						<option value="<?php echo $sncitylist['id']; ?>@@<?php echo $sncitylist['name']; ?>"<?php if($cityId==$sncitylist['id']) echo 'checked'; ?>><?php echo $sncitylist['name']; ?></option>
						<?php
			 }	
			 }
			?>
			</select> <br>
	6.City :&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="text" name="stu_fname" size="35"  style="background-color: #FFFEFC; border:2px solid #9C9A9A; padding: 10px; width:300px
    margin: 5px;" placeholder="">&nbsp;&nbsp;
...
19. Choice Of Course :</td>
 <td width="30%"><select class="soflow-color" name="home_id" >
  <!-- This method is nice because it doesn't require extra div tags, but it also doesn't retain the style across all browsers. -->
...
<td width="30%" style="font-size:30px;">21.Coordinator Id:</td>
 <td><table width="20%">
 	<tr>
<br>
 <input type="text" name="declare" size="50" style="background-color: #FFFEFC; border:2px solid #9C9A9A; padding: 10px ;
    margin: 5px;" placeholder="">
