  ...
  function showimageThumb($imagepath,$w,$h){
   ...
   function showFrontimageThumb($imagepath,$w,$h){
		...
		if(file_exists($imagepath)){
		 ...
		 $imageName=$w."_".basename($imagepath);
		 if(file_exists("thumb/".$imageName)){
		 echo "<img style='border:solid 2px #CCCCCC;padding:4px;cursor:pointer;' src= 'thumb/".$imageName."' border='0'>"; 
		 }else{
		$imageObj=new Thumbnail($imagepath);
		$imageObj->resize($w,$h);
		$imageObj->save("thumb/".$imageName,$w);
		...
		echo "<img style='border:solid 2px #CCCCCC;padding:4px;cursor:pointer;' src= 'images/nophoto.gif' width='".$w."' height='".$h."' border='0'>";  
		}
  }
